...
def updateCells(self, robot, distance, ui):
    ...
    for angle in range(360):
        ...
        findCellOccupancy(self, [end_of_seg_world_x, end_of_seg_world_y], distance[angle], obstacle_is_sensed, robot.robot_position_in_world)

    return self.cells_that_changed

def findCellOccupancy(self, end_of_seg_in_world, current_distance_measurement, obstacle_is_sensed, robot_position):
    """
...
def setCellPOcc(self, current_position, current_distance_measurement, obstacle_is_sensed, robot_position):
    """
    ...
    # Removed to put in inline code
    #probability = getProbability(self, current_distance_measurement, prior_occ, obstacle_is_sensed)
...
def getProbability(self, current_distance_measurement, prior_occ, obstacle_is_sensed):
    """
    ...
    # Calculate Bayesian probability
    if obstacle_is_sensed is True:
        return (POcc * prior_occ) / ((POcc * prior_occ) + (PEmp * inv_prior))
    else:
        return (PEmp * prior_occ) / ((PEmp * prior_occ) + (POcc * inv_prior))
